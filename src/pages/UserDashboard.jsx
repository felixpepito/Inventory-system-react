    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 lg:ml-64 transition-all duration-300">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold tracking-tight text-gray-900">
                {activePage === 'inventory' && 'Tire Inventory by Brand'}
        ...
        <div className="px-6 py-6 lg:px-8">
          {/* INVENTORY VIEW - Grouped by Brand in Vertical Format */}
          ...
          {/* BRANDS VIEW - Fixed Search */}
          {activePage === 'brands' && (
            <div className="space-y-6">
              {/* Search Input - Same as inventory */}
              <div className="relative max-w-md">
                ...
                <input
                  type="text"
                  placeholder="Search by brand name, ID, or description..."
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  className="w-full rounded-md border border-gray-300 py-2 pl-9 pr-8 text-sm placeholder:text-gray-400 focus:border-black focus:outline-none focus:ring-1 focus:ring-black"
                />
              ...
              {/* Brand Cards - Filtered by search */}
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                {getFilteredBrands().map((brand) => {
                  const brandTires = getTiresByBrand(brand)
                  const filteredBrandTires = getFilteredTiresBySearch(brandTires)
                  const totalQty = filteredBrandTires.reduce((sum, t) => sum + (t.quantity || 0), 0)
                  const lowStock = filteredBrandTires.filter(t => t.quantity < 5).length
                  const isSelected = selectedBrand === brand
                  return (
                    <div
                      key={brand}
                    ...
                    >
                      <div className={`h-1 w-full rounded-t-md transition-colors ${isSelected ? 'bg-black' : 'bg-gray-200 group-hover:bg-gray-300'}`}></div>
                      <div className="p-4">
                        <div className="flex items-center justify-between">
                          <span className="font-semibold text-gray-900">{brand}</span>
                          {isSelected && <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-black"><path d="M20 6L9 17l-5-5"/></svg>}
                        </div>
                        <div className="mt-3 flex items-center justify-between border-t border-gray-100 pt-3">
                          <div><div className="text-xs text-gray-500">Items</div><div className="text-lg font-semibold text-gray-900">{filteredBrandTires.length}</div></div>
                          <div className="h-6 w-px bg-gray-200"></div>
                          <div><div className="text-xs text-gray-500">Qty</div><div className="text-lg font-semibold text-gray-900">{totalQty.toLocaleString()}</div></div>
                        </div>
                        {lowStock > 0 && (
              ...
              {getFilteredBrands().length === 0 && (
                <div className="flex flex-col items-center justify-center rounded-md border border-gray-200 bg-white py-12">
                  ...
                  <p className="mt-4 text-sm font-medium text-gray-900">No brands found</p>
                  <p className="mt-1 text-xs text-gray-500">Try adjusting your search term</p>
                </div>
              )}

              {/* Brands Inventory Table - Filtered by search and selected brand */}
              {getFilteredBrands().length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500">
                      ({selectedBrand ? getFilteredTiresBySearch(getTiresByBrand(selectedBrand)).length : getFilteredTiresBySearch(tires).length})
                    </span>
                  </div>
                  <InventoryTable
                    tires={selectedBrand ? getFilteredTiresBySearch(getTiresByBrand(selectedBrand)) : getFilteredTiresBySearch(tires)}
                    isAdmin={false}
                    onEdit={null}
